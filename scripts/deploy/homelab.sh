...
IMPORT_NODE="${IMPORT_NODE:?IMPORT_NODE is required for sideload (e.g. homelab-worker-01)}"
...
if [ -n "$GATEWAY_IP" ]; then
  ...
  echo "WARN: Gateway has no LoadBalancer IP — check MetalLB and Gateway controller"
fi
