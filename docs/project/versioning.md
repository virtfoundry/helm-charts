...
The project is **not 1.0 yet**. The current release line is **0.7.2**.
...
Helm chart packages `1.x` were **yanked** from the repository index so a bare `helm install` resolves **0.7.2**. Pin anyway and install **operator first**:

```bash
helm install virtfoundry-operator virtfoundry/virtfoundry-operator --version 0.7.2 \
  -n virtfoundry-system --create-namespace
helm install virtfoundry virtfoundry/virtfoundry --version 0.7.2 \
...
```bash
# Helm — pin 0.7.2; install operator first (see Installation guide)
helm install virtfoundry-operator virtfoundry/virtfoundry-operator --version 0.7.2 ...
helm install virtfoundry virtfoundry/virtfoundry --version 0.7.2 \
  --set secrets.rootPassword='...' --set secrets.jwtSecret='...' ...

# Container images
ghcr.io/virtfoundry/core:0.7.2
ghcr.io/virtfoundry/ui:0.7.2
ghcr.io/virtfoundry/operator:0.7.2
```
