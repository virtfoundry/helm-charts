...
|--------|--------------|---------|
| Bug fix, doc fix, security patch (non-breaking) | PATCH | `0.7.1` → `0.7.3` |
| New feature, chart profile change | MINOR | `0.7.3` → `0.8.0` |
| Breaking API or chart contract | MINOR while on 0.x | `0.7.3` → `0.8.0` (document in CHANGELOG) |
...
```bash
# Helm — pin 0.7.3; install operator first (see Installation guide)
helm install virtfoundry-operator virtfoundry/virtfoundry-operator --version 0.7.3 ...
helm install virtfoundry virtfoundry/virtfoundry --version 0.7.3 \
  --set secrets.rootPassword='...' --set secrets.jwtSecret='...' ...

# Container images
ghcr.io/virtfoundry/core:0.7.3
ghcr.io/virtfoundry/ui:0.7.3
ghcr.io/virtfoundry/operator:0.7.3
```
